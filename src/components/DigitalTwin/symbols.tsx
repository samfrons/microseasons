/**
 * Drawing primitives for the sheet plate — plain React SVG, no d3, so every
 * sheet renders on the server. Ink comes from the ACTIVE PALETTE
 * (`usePalette()`); a symbol never inlines a colour and never imports
 * `BLUEPRINT` directly, which is what lets one sheet be Blueprint and the
 * next Classic on the same page. Add ONE primitive here and ONE case in the
 * renderer when a symbol is missing; never hand-draw a sheet.
 */
import type { ReactNode } from 'react';
import { LINE_STYLE, MIN_TEXT_OPACITY, MONO_FONT, WEIGHT, type LineKind, type Palette } from '@/lib/twin/blueprint';
import type { Pt, TitleBlock, LogicBlock, Part, InstrumentMount } from '@/lib/twin/spec';
import { defId, usePalette, useTwinTheme } from './theme';

/** Opaque body fill + ink outline, from the active palette. */
function bodyProps(p: Palette) {
  return { fill: p.body, stroke: p.ink, strokeWidth: WEIGHT.process } as const;
}
...
export function Poly({ pts, kind = 'process', live = false, arrow = false, stroke }: {
  ...
  return (
    <polyline
      ...
      fill="none"
      stroke={stroke ?? (live ? p.live : p.ink)}
      strokeWidth={live && !isSignal ? WEIGHT.live : st.width}
      strokeDasharray={st.dash}
      strokeLinejoin="miter"
      strokeLinecap="butt"
      markerEnd={arrow ? `url(#${defId(theme, live ? 'tw-arrow-live' : 'tw-arrow')})` : undefined}
    />
  );
}
...
export function Leader({ from, to }: { from: Pt; to: Pt }) {
  const p = usePalette();
  return <line x1={from[0]} y1={from[1]} x2={to[0]} y2={to[1]} stroke={p.ink} strokeWidth={WEIGHT.fine} />;
}
...
export function Defs() {
  ...
  return (
    <defs>
      <marker id={defId(theme, 'tw-arrow')} markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto" markerUnits="userSpaceOnUse">
        <path d="M0,0 L8,4 L0,8 z" fill={p.ink} />
      </marker>
      <marker id={defId(theme, 'tw-arrow-live')} markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto" markerUnits="userSpaceOnUse">
        <path d="M0,0 L8,4 L0,8 z" fill={p.live} />
      </marker>
      <pattern id={defId(theme, 'tw-hatch')} width="4" height="4" patternUnits="userSpaceOnUse" patternTransform="rotate(45)">
        <line x1="0" y1="0" x2="0" y2="4" stroke={p.ink} strokeWidth="0.6" />
      </pattern>
      <pattern id={defId(theme, 'tw-mesh')} width="3" height="3" patternUnits="userSpaceOnUse">
        <path d="M0 1.5 H3 M1.5 0 V3" stroke={p.ink} strokeWidth="0.35" />
      </pattern>
    </defs>
  );
}
...
export function Chamber({ x, y, w, h, live = false, tint, filled = true }: {
  ...
  return (
    <g>
      <rect x={x - w / 2} y={y - h / 2} width={w} height={h} {...bodyProps(p)} stroke={live ? p.live : p.ink} strokeWidth={live ? WEIGHT.live : WEIGHT.process} />
      {tint && filled && (
...
export function Vessel({ x, y, w, h, live = false }: { x: number; y: number; w: number; h: number; live?: boolean }) {
  ...
  return (
    <g>
      <path d={d} {...bodyProps(p)} stroke={live ? p.live : p.ink} strokeWidth={live ? WEIGHT.live : WEIGHT.process} />
      <line x1={x - r + 4} y1={y + h * 0.15} x2={x + r - 4} y2={y + h * 0.15} stroke={p.ink} strokeWidth={WEIGHT.fine} strokeDasharray="2 2" />
    </g>
  );
}
...
export function Pump({ x, y, live = false }: { x: number; y: number; live?: boolean }) {
  ...
  return (
    <g>
      <circle cx={x} cy={y} r={14} {...bodyProps(p)} stroke={live ? p.live : p.ink} />
      {/* peristaltic: three rollers on the rotor */}
      {[0, 120, 240].map((a) => {
        const rad = (a * Math.PI) / 180;
        return <circle key={a} cx={x + Math.cos(rad) * 7} cy={y + Math.sin(rad) * 7} r={2.2} fill="none" stroke={p.ink} strokeWidth={WEIGHT.fine} />;
      })}
      <circle cx={x} cy={y} r={1.5} fill={p.ink} />
    </g>
  );
}
...
export function Valve({ x, y, open, actuated = false, vertical = false }: { x: number; y: number; open: boolean; actuated?: boolean; vertical?: boolean }) {
  ...
  return (
    <g>
      <path d={d} fill={open ? p.body : p.ink} stroke={open ? p.live : p.ink} strokeWidth={open ? WEIGHT.live : WEIGHT.process} />
      {actuated && (
        <g>
          <line x1={x} y1={y} x2={x} y2={y - s - 6} stroke={p.ink} strokeWidth={WEIGHT.fine} />
          <path d={`M${x - 7},${y - s - 6} A7,4 0 0 1 ${x + 7},${y - s - 6} Z`} fill={p.body} stroke={p.ink} strokeWidth={WEIGHT.fine} />
        </g>
      )}
    </g>
  );
...
export function Membrane({ x, y, w, h }: { x: number; y: number; w: number; h: number }) {
  const p = usePalette();
  const theme = useTwinTheme();
  return <rect x={x - w / 2} y={y - h / 2} width={w} height={h} fill={`url(#${defId(theme, 'tw-hatch')})`} stroke={p.ink} strokeWidth={WEIGHT.fine} />;
}
...
export function Electrode({ x, y, w, h, polarity, live = false }: { x: number; y: number; w: number; h: number; polarity: 'anode' | 'cathode'; live?: boolean }) {
  ...
  return (
    <g>
      <rect x={x - w / 2} y={y - h / 2} width={w} height={h} fill={live ? p.live : p.ink} opacity={live ? 1 : 0.9} />
...
export function Led({ x, y, w, h, on }: { x: number; y: number; w: number; h: number; on: boolean }) {
  ...
  return (
    <g>
      <rect x={x - w / 2} y={y - h / 2} width={w} height={h} {...bodyProps(p)} stroke={on ? p.live : p.ink} />
      {Array.from({ length: n }, (_, i) => {
        const cx = x - w / 2 + (w / n) * (i + 0.5);
        return <rect key={i} x={cx - 2} y={y - 2} width={4} height={4} fill={on ? p.live : 'none'} stroke={p.ink} strokeWidth={WEIGHT.fine} />;
      })}
      {on && [-1, 0, 1].map((k) => (
        <line key={k} x1={x + k * 8} y1={y - h / 2 - 2} x2={x + k * 10} y2={y - h / 2 - 8} stroke={p.live} strokeWidth={WEIGHT.fine} />
      ))}
    </g>
  );
}
...
export function PowerSupply({ x, y, w, h, live = false }: { x: number; y: number; w: number; h: number; live?: boolean }) {
  ...
  return (
    <g>
      <rect x={x - w / 2} y={y - h / 2} width={w} height={h} {...bodyProps(p)} stroke={live ? p.live : p.ink} />
...
export function Converter({ x, y, w, h, live = false }: { x: number; y: number; w: number; h: number; live?: boolean }) {
  ...
  return (
    <g>
      <rect x={x - w / 2} y={y - h / 2} width={w} height={h} {...bodyProps(p)} stroke={live ? p.live : p.ink} />
      <line x1={x - w / 2} y1={y + h / 2} x2={x + w / 2} y2={y - h / 2} stroke={p.ink} strokeWidth={WEIGHT.fine} />
...
export function Storage({ x, y, w, live = false }: { x: number; y: number; w: number; live?: boolean }) {
  const p = usePalette();
  const c = live ? p.live : p.ink;
  return (
    <g>
      <line x1={x - w / 2} y1={y - 3} x2={x + w / 2} y2={y - 3} stroke={c} strokeWidth={WEIGHT.process} />
      <line x1={x - w / 2} y1={y + 3} x2={x + w / 2} y2={y + 3} stroke={c} strokeWidth={WEIGHT.process} />
      <line x1={x} y1={y - 3} x2={x} y2={y - 8} stroke={p.ink} strokeWidth={WEIGHT.electric} />
      <line x1={x} y1={y + 3} x2={x} y2={y + 8} stroke={p.ink} strokeWidth={WEIGHT.electric} />
    </g>
  );
}
...
export function Switch({ x, y, w, closed, vertical = false }: { x: number; y: number; w: number; closed: boolean; vertical?: boolean }) {
  ...
  return (
    <g>
      <circle cx={a[0]} cy={a[1]} r={1.6} fill={p.ink} />
      <circle cx={b[0]} cy={b[1]} r={1.6} fill={p.ink} />
      <line x1={a[0]} y1={a[1]} x2={blade[0]} y2={blade[1]} stroke={closed ? p.live : p.ink} strokeWidth={WEIGHT.process} />
    </g>
  );
}
    ...
    <g>
      ...
      {Array.from({ length: rows }, (_, r) =>
        Array.from({ length: cols }, (_, c) => (
          <rect key={`${r}-${c}`} x={x - w / 2 + 6 + c * cw + 1} y={y - h / 2 + 6 + r * ch + 1} width={cw - 2} height={ch - 2} fill="none" stroke={p.ink} strokeWidth={WEIGHT.fine} opacity={0.8} />
        ))
      )}
    </g>
  );
...
export function Filter({ x, y, w, h }: { x: number; y: number; w: number; h: number }) {
  ...
  return (
    <g>
      <rect x={x - w / 2} y={y - h / 2} width={w} height={h} {...bodyProps(p)} />
      <rect x={x - w / 2 + 3} y={y - h / 2 + 3} width={w - 6} height={h - 6} fill={`url(#${defId(theme, 'tw-mesh')})`} />
    </g>
  );
}
...
export function SamplePoint({ x, y }: { x: number; y: number }) {
  ...
  return (
    <g>
      <circle cx={x} cy={y} r={5} {...bodyProps(p)} />
...
export function Cabinet({ x, y, w, h }: { x: number; y: number; w: number; h: number }) {
  ...
  return (
    <g>
      <rect x={x - w / 2} y={y - h / 2} width={w} height={h} {...bodyProps(p)} />
      <rect x={x - w / 2 + 4} y={y - h / 2 + 4} width={w - 8} height={h - 8} fill="none" stroke={p.ink} strokeWidth={WEIGHT.fine} strokeDasharray="2 2" />
    </g>
  );
}
...
export function OffSheet({ x, y, kind, label, live = false }: { x: number; y: number; kind: 'source' | 'sink'; label?: string; live?: boolean }) {
  ...
  return (
    <g>
      <path d={d} fill={p.body} stroke={live ? p.live : p.ink} strokeWidth={WEIGHT.fine} />
...
export function InstrumentBubble({ x, y, tag, mount = 'field', tip }: { x: number; y: number; tag: string; mount?: InstrumentMount; tip?: string }) {
  ...
  return (
    <g aria-label={tip ?? tag}>
      ...
      {mount === 'dcs' && <rect x={x - r - 2} y={y - r - 2} width={2 * r + 4} height={2 * r + 4} fill={p.bubble} stroke={p.ink} strokeWidth={WEIGHT.fine} />}
      <circle cx={x} cy={y} r={r} fill={p.bubble} stroke={p.ink} strokeWidth={WEIGHT.process} />
      {mount === 'panel' && <line x1={x - r} y1={y} x2={x + r} y2={y} stroke={p.ink} strokeWidth={WEIGHT.process} />}
      <line x1={x - r} y1={y} x2={x + r} y2={y} stroke={p.ink} strokeWidth={WEIGHT.fine} />
...
export function Balloon({ x, y, n, to }: { x: number; y: number; n: number; to: Pt }) {
  ...
  return (
    <g>
      ...
      <circle cx={x} cy={y} r={8} fill={p.bubble} stroke={p.ink} strokeWidth={WEIGHT.fine} />
...
export function Stamp({ x, y, lines }: { x: number; y: number; lines: [string, string] }) {
  ...
  return (
    <g transform={`rotate(-4 ${x} ${y})`}>
      <rect x={x - 96} y={y - 13} width={192} height={26} fill={p.stampPaper} stroke={p.stampInk} strokeWidth={1.2} />
      <text x={x} y={y - 2} fontFamily={MONO_FONT} fontSize={7} fontWeight={700} textAnchor="middle" fill={p.stampInk} letterSpacing=".12em">{lines[0]}</text>
      <text x={x} y={y + 8} fontFamily={MONO_FONT} fontSize={6} textAnchor="middle" fill={p.stampInk} letterSpacing=".1em">{lines[1]}</text>
    </g>
  );
}
...
export function TitleBlockBox({ x, y, w = 336, t }: { x: number; y: number; w?: number; t: TitleBlock }) {
  ...
  return (
    <g>
      <rect x={x} y={y} width={w} height={h} fill={p.panel} stroke={p.ink} strokeWidth={WEIGHT.process} />
      <line x1={x} y1={y + 30} x2={x + w} y2={y + 30} stroke={p.ink} strokeWidth={WEIGHT.fine} />
      <line x1={x + w - 130} y1={y + 30} x2={x + w - 130} y2={y + h} stroke={p.ink} strokeWidth={WEIGHT.fine} />
  ...
  return (
    <g>
      ...
      {parts.map((q, i) => {
        ...
        return (
          <g key={q.n}>
            <circle cx={x + c * 240 + 6} cy={y + 12 + r * 11 - 2} r={5} fill={p.bubble} stroke={p.ink} strokeWidth={WEIGHT.fine} />
            <Txt x={x + c * 240 + 6} y={y + 12 + r * 11} size={5.5} anchor="middle" bright>{q.n}</Txt>
            <Txt x={x + c * 240 + 16} y={y + 12 + r * 11} size={6}>{q.name.toUpperCase()}</Txt>
          </g>
        );
      })}
    </g>
...
export function LogicBlockBox({ block, activeRule, stamp }: { block: LogicBlock; activeRule?: string; stamp?: string }) {
  ...
  return (
    <g>
      ...
      <line x1={x} y1={y + 19} x2={x + w} y2={y + 19} stroke={p.ink} strokeWidth={WEIGHT.fine} />
      {rules.map((r, i) => {
        const live = r.id === activeRule;
        const ry = y + 31 + i * 13;
        return (
          <g key={r.id}>
            {live && <rect x={x + 3} y={ry - 9} width={w - 6} height={12} fill={p.live} opacity={0.18} />}
            <rect x={x + 8} y={ry - 6} width={5} height={5} fill={live ? p.live : 'none'} stroke={p.ink} strokeWidth={WEIGHT.fine} />
...
export function SheetFrame({ w, h }: { w: number; h: number }) {
  const p = usePalette();
  return <rect x={0.5} y={0.5} width={w - 1} height={h - 1} fill="none" stroke={p.frame} strokeWidth={WEIGHT.fine} opacity={p.frameOpacity} />;
}
